...
static void *wait_exit_thread(void *arg)
{
    ...
    // LIMOUSINE PATCH (vs. upstream flutter_pty 0.4.2):
    // Original code called `waitpid(pid, &status, 0)` once with no
    // return-value check. On macOS, SIGCHLD delivery (raised whenever ANY
    // child of the process changes state — including unrelated subprocesses
    // spawned via Dart's Process.run/start) can interrupt the blocking call,
    // making it return -1 with errno=EINTR. With the unchecked status word
    // still uninitialised, WIFEXITED/WIFSIGNALED could spuriously match
    // garbage bits and post a bogus "exit code" to Dart — which closes the
    // PTY's output port and makes a live child look terminated. Linux's
    // glibc usually masks this via SA_RESTART on the syscall; macOS doesn't.
    //
    // Fix: standard POSIX EINTR retry + check the return value before
    // interpreting `status`. Also free `options` to plug a per-PTY leak.
    int status = 0;
    pid_t wp;
    do {
        wp = waitpid(options->pid, &status, 0);
